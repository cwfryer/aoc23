...
module Row = struct
  type t = { diagram : char list; layout : int list }
    ...
    let rec solve' diagram layout =
      let res =
        ...
        | None -> (
            ...
            | '?' :: ss, _ ->
                solve' ('#' :: ss) layout + solve' ('.' :: ss) layout
...
module Part_1 = struct
  let run (input : string) : (string, string) Result.t =
    let@ (machine : Machine.t) =
      Angstrom.(parse_string ~consume:All machine input)
    in
    let answer =
  ...
  let run (input : string) : (string, string) Result.t =
    ...
    let machine =
      List.map machine ~f:(fun row ->
          let diagram =
            row.diagram |> List.append [ '?' ] |> List.append row.diagram
            |> List.append [ '?' ] |> List.append row.diagram
            |> List.append [ '?' ] |> List.append row.diagram
            |> List.append [ '?' ] |> List.append row.diagram
          in
          let layout =
            row.layout |> List.append row.layout |> List.append row.layout
            |> List.append row.layout |> List.append row.layout
          in
          Row.{ diagram; layout })
    in
    let answer =
