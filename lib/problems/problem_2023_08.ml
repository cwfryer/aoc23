...
module Document = struct
  ...
  let follow_instructions (t : t) : int =
    let rec go (acc : int) (node : Node.t) : Instructions.t -> int = function
      ...
      | x :: xs ->
        if String.equal node.label "ZZZ"
        then acc
        else (
          let new_label =
            match x with
            | Left -> node.left_label
            | Right -> node.right_label
          in
  ...
  let ghost_follow_instructions (t : t) : int =
    let a_nodes =
      ...
      |> List.map ~f:snd
    in
    let rec go (acc : int) (nodes : Node.t list) : Instructions.t -> int = function
...
module Angstrom = (* {{{ *) struct
  ...
  let graph : Graph.t t =
    ...
    let graph =
      nodes |> List.map ~f:(fun (x : Node.t) -> x.label, x) |> Graph.Map.of_alist
    in
    match graph with
