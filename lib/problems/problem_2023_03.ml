...
module Table = struct
  ...
  let rec seek_to_start_of_part_number (t : t) ((x, y) : int * int) : int * int =
    ...
    | None -> x, y
    | Some c ->
      if Char.is_digit c then seek_to_start_of_part_number t (x - 1, y) else x, y
  ;;
  ...
  let part_numbers_around_symbol (t : t) ((x, y) : int * int) : int list =
    neighbors t (x, y)
    |> List.filter_map ~f:(fun (x, y, c) -> if Char.is_digit c then Some (x, y) else None)
  ...
  let of_input (input : string) : char Table.t =
    let entries =
      ...
      let* y, line =
        ...
        match
          List.zip (List.range ~start:`inclusive ~stop:`exclusive 0 (List.length ls)) ls
        with
...
module Part_1 = struct
  let run (input : string) : (string, string) result =
    ...
    in
    Ok (string_of_int @@ List.fold all_part_numbers ~init:0 ~f:( + ))
  ;;
end
...
module Part_2 = struct
  let run (input : string) : (string, string) result =
    ...
    Ok (string_of_int @@ List.fold ratios ~init:0 ~f:( + ))
  ;;
end
