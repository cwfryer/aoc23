...
module Almanac = struct
  ...
  let eval_map_row : int * int * int -> int -> int option =
    ...
    if n >= src_start && n < src_start + length
    then Some (dst_start + (n - src_start))
    else None
  ;;
  ...
  let min_in_range (start : int) (end_ : int) ~(f : int -> int) : int =
  ...
  let eval_seed_ranges (t : t) : int =
    let rec go (min_so_far : int) = function
      ...
      | s1 :: s2 :: ss ->
        Stdio.printf "Computing range %d, %d\n%!" s1 s2;
...
module Part_1 = struct
  let run (input : string) : (string, string) result =
    ...
    Ok
      (string_of_int
       @@ Option.value ~default:0 (List.min_elt ~compare:Int.compare results))
  ;;
end
