...
module Grid = struct
  ...
  type beam = { coords : position; direction : direction }
  type point = { coords : position; shape : char; energized : bool }
  type t = point Grid.t
  ...
  let of_string s : t =
    let grid =
      ...
      |> List.mapi ~f:(fun i line ->
             ...
             |> Array.mapi ~f:(fun j el ->
                    { coords = (j, i); shape = el; energized = false }))
      |> Array.of_list
    in
    grid
  ...
  let crawl (t : t) b : t =
    let rec run_beam_until_end beams t acc =
      ...
      | [] -> t
      | b :: bs ->
          if acc > 10 then run_beam_until_end bs t 0
          else
            let next = move b.direction b.coords in
            if inside t next then (
  ...
  let entry_points t : beam list =
    let left_side =
      List.init (Grid.height t) ~f:(fun r ->
          { coords = (r, -1); direction = E })
    in
    let right_side =
      List.init (Grid.height t) ~f:(fun r ->
          { coords = (r, Grid.width t); direction = W })
    in
    let top_side =
      List.init (Grid.width t) ~f:(fun c -> { coords = (-1, c); direction = S })
    in
    let bot_side =
      List.init (Grid.width t) ~f:(fun c ->
          { coords = (Grid.height t, c); direction = N })
    in
...
module Part_1 = struct
  let run (input : string) : (string, string) Result.t =
    ...
    let answer =
      Grid.fold
        (fun (_x, _y) (el : Grid.point) acc ->
          if el.energized then acc + 1 else acc)
        grid' 0
      |> Int.to_string
    in
    Ok answer
