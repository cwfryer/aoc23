...
module Int = struct
  ...
  let maximum (xs : int list) : int = List.fold_left xs ~f:max ~init:Int.min_value
  let minimum (xs : int list) : int = List.fold_left xs ~f:min ~init:Int.max_value
end
...
module Table = struct
  ...
  module Make (K1 : KEY) (K2 : KEY) : T with type K1.t = K1.t and type K2.t = K2.t =
  struct
    module K1 = K1
    module K2 = K2
    ...
    let contains ((k1, k2) : K1.t * K2.t) (m2 : 'v t) : bool =
      match Map2.find_opt k2 m2 with
      | None -> false
      | Some m1 -> Map1.mem k1 m1
    ;;
    ...
    let remove ((k1, k2) : K1.t * K2.t) (m : 'v t) : 'v t =
      Map2.update
        ...
        (function
          | None -> None
          | Some m1 -> Some (Map1.remove k1 m1))
        m
    ;;

    let update ((k1, k2) : K1.t * K2.t) (f : 'v option -> 'v option) (m : 'v t) : 'v t =
...
module Angstrom = struct
  ...
  let lowercase_ascii : char t =
    choice @@ List.init 26 ~f:(fun x -> char @@ char_of_int (x + int_of_char 'a'))
  ;;
  ...
  let lowercase_hex : char t =
    ...
    <|> choice (List.init 10 ~f:(fun x -> char @@ char_of_int (x + int_of_char '0')))
  ;;
  ...
  let read c =
    let rec scan rows =
      ...
      | exception End_of_file ->
        ...
        let g = Array.map (Array.of_list (List.rev rows)) ~f:(fun r -> row r) in
